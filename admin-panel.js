...
class AdminPanel {
    ...
    async init() {
        ...
        try {
            ...
            // Last mein real-time setup karo
            ...
            this.showNotification('Admin panel initialization failed: ' + error.message, 'error');
        }
    }
    ...
    async waitForSupabase() {
        ...
        while (attempts < maxAttempts) {
            if (window.supabase && typeof window.supabase.from === 'function') {
                console.log('✅ Supabase properly initialized!');
                ...
                try {
                    ...
                    console.log('⚠️ Supabase connected but table might not exist');
                    return true;
                }
            }
            
            console.log(`🔄 Waiting for Supabase to initialize... (${attempts + 1}/${maxAttempts})`);
    ...
    setupEventListeners() {
        ...
        if (priceInput) priceInput.addEventListener('input', this.calculateDiscount.bind(this));
        if (originalPriceInput) originalPriceInput.addEventListener('input', this.calculateDiscount.bind(this));
    }
    ...
    async loadProductsFromSupabase() {
        try {
            if (!window.supabase || typeof window.supabase.from !== 'function') {
                console.error('❌ Supabase not initialized for products');
                return;
            }
            ...
            if (error) {
                ...
                if (error.code === '42P01') {
                    this.showNotification('Products table not created. Please run SQL in Supabase dashboard.', 'error');
                    return;
                }
                throw error;
            }
    ...
    async loadOrdersFromSupabase() {
        try {
            if (!window.supabase || typeof window.supabase.from !== 'function') {
                console.error('❌ Supabase not initialized for orders');
                return;
            }
            ...
            if (error) {
                ...
                if (error.code === '42P01') {
                    this.showNotification('Orders table not created. Please run SQL in Supabase dashboard.', 'error');
                    return;
                }
                throw error;
            }
    ...
    async loadReturnRequestsFromSupabase() {
        try {
            if (!window.supabase || typeof window.supabase.from !== 'function') {
                console.error('❌ Supabase not initialized for returns');
                return;
            }
            
            console.log('📥 Loading return requests from Supabase...');
            const { data: returns, error } = await window.supabase
            ...
            if (error) {
                if (error.code === 'PGRST205' || error.code === '42P01') {
                    console.log('📋 Return requests table not created yet');
                    this.returnRequests = [];
                    return;
                }
                throw error;
            ...
            console.log(`✅ Loaded ${this.returnRequests.length} return requests`);
    ...
    // ✅ REAL-TIME SUBSCRIPTIONS SETUP KARO
    setupRealtimeSubscriptions() {
        console.log('🔔 Setting up real-time subscriptions...');
        
        // Check if window.supabase exists
        if (!window.supabase || typeof window.supabase.channel !== 'function') {
            console.error('❌ window.supabase not available for real-time');
            return;
        }
        
        try {
            console.log('✅ Setting up orders real-time channel...');
            
            // Orders ke liye real-time updates - ONLY window.supabase use karo
            window.supabase
            ...
            console.log('✅ Setting up returns real-time channel...');
                
            // Return requests ke liye real-time updates - ONLY window.supabase use karo  
            window.supabase
            ...
            console.log('⚠️ Real-time subscriptions failed:', error.message);
        }
    }
        ...
        try {
            ...
            if (error) {
                ...
                if (error.code === '42P01') {
                    ...
                    this.showNotification('Permission denied. Check RLS policies.', 'error');
                } else {
            ...
            this.showNotification('Error adding product. Check console for details.', 'error');
        }
    }
